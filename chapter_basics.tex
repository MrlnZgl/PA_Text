...
EasyPBC is an \name{Abaqus} plugin to automatically create \acrfull{pbc}. The plugin is developed by XX. The plugin is no official tool, thus it is not online available. Due to the utilisation in previous works, the plugin was easily available. EasyPBC is a tool which creates \acrshort{pbc} via constraints in \name{Abaqus}
