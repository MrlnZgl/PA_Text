...
In general, the trend of the plastic yield, alpha and beta ends in all tests to optimisation results similar to the corresponding reference values. For the elastic parameters, we observe contrary behaviour within all tests, where either the reference value for $E$ matches or the reference value for $\nu$. To verify the quality of the \acrlong{omp}, the load reactions are considered. 
...
For both mixing ratios, the values for the plastic yield, alpha and beta were determined within a small range of variations for all tests. All theses determined values are close to their respective reference value.
