    ...
    To perform the comparison of material behaviours we need analogous data from the \name{abaqus} simulation. From the odb we read out stress and strain values in all directions. Similar to the reference load reactions we extract the values corresponding to the evaluation measurements. These values are called optimized load reactions (olr). \\
